...
async function loadCashMoves(dayStr) {
  ...
  try {
    ...
    listEl.innerHTML = moves.map(m => {
      ...
      const colorClass = isOut ? "text-danger" : "text-info";
  ...
  try {
    ...
    checksManagementRows.innerHTML = checks.map(row => `
      <tr data-check-id="${row.id}" class="check-row-clickable" tabindex="0" role="button" aria-label="Apri storico assegno">
        ...
        <td class="text-end">${formatEuro2(row.amount || 0)}</td>
        <td class="text-end">
          <button type="button" class="btn btn-sm btn-info btn-check-history" data-id="${row.id}">Stato e storico</button>
          <button type="button" class="btn btn-sm btn-outline-secondary btn-check-edit" data-row='${escapeHtml(JSON.stringify(row))}'>Modifica dati</button>
...
function renderCheckHistory(check) {
  if (!check) return;
  if (checkHistorySubtitle) {
  ...
  if (checkEventStatus) {
    checkEventStatus.innerHTML = transitions.length
      ...
      : '<option value="">Nessuna transizione disponibile</option>';
    checkEventStatus.disabled = !transitions.length;
  }
      ...
      return `
        <article class="check-history-event">
          ...
          <div class="check-history-event__body">
            <strong>${escapeHtml(fromLabel)} → ${escapeHtml(event.to_status_label || event.to_status || "")}</strong>
...
function updateCheckEventPenalty() {
  if (!checkEventPenalty) return;
  const penalty = checkEventStatus?.value === "protested"
...
async function saveCheckStatusEvent() {
  ...
  try {
    ...
    if (checkEventNote) checkEventNote.value = "";
    await loadChecksManagement();
    if (currentDay) await refreshAgendaSections(["preview", "incassi", "spese", "assegni"]);
  } catch (error) {
    alert(error.message || "Errore cambio stato assegno");
  } finally {
    if (checkEventSaveBtn) {
...
document.addEventListener("DOMContentLoaded", async function () {
      ...
      spicciTableBody.innerHTML = rows.map(row => {
        ...
        return `
          <tr data-spicci-id="${row.id}">
            ...
            <td>${escapeHtml(row.notes || "")}</td>
            <td class="text-end ${row.direction === "out" ? "text-danger" : "text-info"}">
      ...
      depositTableBody.innerHTML = rows.map(row => `
        <tr data-deposit-id="${row.id}">
          ...
          <td class="text-end">
            <button
              type="button"
              class="btn btn-outline-info me-1 btn-sm btn-edit-deposit"
              data-id="${row.id}">
              Modifica
            </button>
            <button
  ...
  async function saveOperation() {
    ...
    let built;
    try {
      built = await buildOperationPayload();
    } catch (err) {
      console.error("buildOperationPayload error:", err);
      alert(err.message || "Errore durante la preparazione dell'operazione.");
    ...
    tbody.innerHTML = rows.map(row => {
      ...
      return `
        ...
        >
          ...
          <td class="text-end">
            <button type="button" class="btn btn-sm btn-outline-info movement-search-edit" ${canEdit ? "" : "disabled"}>Apri</button>
  ...
  checksManagementRows?.addEventListener("click", async (e) => {
    ...
    if (historyBtn) {
      try {
        await openCheckHistory(historyBtn.dataset.id);
      } catch (err) {
        console.error("check history error:", err);
        alert(err.message || "Errore caricamento storico assegno");
      }
      return;
    }
  ...
  (function initCustomerSuggest() {
    ...
    input.addEventListener("change", async () => {
      ...
      if (chosen.registry_id) {
        if (hiddenRegistryId) hiddenRegistryId.value = String(chosen.registry_id);
      }
    });
  })();
  ...
  (function initCustomerSearchModal() {
    ...
    btnConfirm.addEventListener("click", async () => {
      ...
      try {
        ...
        if (opHiddenRegistryId) opHiddenRegistryId.value = selectedItem.kind === "customer" && selectedItem.registry_id && !selectedItem.id ? String(selectedItem.registry_id) : "";
