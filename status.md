...
- Aggiunti i token per i divisori indipendenti navbar/corpo e corpo/footer, con colore e spessore configurabili e anteprima immediata.
- I divisori ora espongono tre modalità reali: `pennellata` con le immagini storiche, `linea` oppure `nessun divisorio`; il preset LDApp mantiene la pennellata attuale.
...
- I valori responsive della Home sono stati ricondotti a token standard (`page_max_width`, `action_min_height`, `action_gap`, `mobile_action_columns`) esposti nel tema e riutilizzabili da componenti analoghi.
...
## 2026-09-19 - Gestione carte aziendali

- Aggiunta la tabella `company_credit_cards`, il tile Impostazioni e la pagina per aggiungere, modificare, attivare/disattivare ed eliminare le carte.
- La cancellazione viene rifiutata se esistono movimenti in `cash_expense_payments` che conservano quel nome carta; le carte disattivate restano disponibili nello storico ma non vengono proposte in Agenda.
