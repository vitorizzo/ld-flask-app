...
- Verifica pubblica del 2026-08-17: il deploy risponde effettivamente `303` e crea l'intento anonimo. Corretto quindi il passaggio Android residuo: `static/service-worker.js` intercetta esplicitamente la navigazione `POST /pwa/share` e la inoltra seguendo i redirect, invece di ignorare tutte le richieste non-GET. Cache worker portata a `v24` e URL di registrazione a `mobile12` per distribuire automaticamente il nuovo handler alle PWA installate.
