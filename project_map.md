...
- Il profilo contabile passa al worker un read timeout dedicato di 180 secondi; il client globale non altera il comportamento del test di connessione `1000`.
...
- Gli esiti negativi vengono tradotti in messaggi italiani comprensibili e conservano l'eventuale `codiceEsito`. Le partite restano associate all'azione fallita finché il cliente sceglie `Riprendi pagamento` o `Annulla pagamento`.
- `Riprendi pagamento` verifica autorizzazione, ultimo snapshot, identità/importi delle partite e crea una nuova pratica con un nuovo `codTrans` Nexi; la transazione rifiutata non viene mai riutilizzata. `Annulla pagamento` libera esplicitamente le partite.
- Le partite con un'azione attiva restano selezionabili nella situazione contabile. La barra di selezione descrive lo stato, disabilita le nuove operazioni incompatibili e, quando consentito, espone `Annulla azione precedente` con conferma.
...
- `routes/registry.py`, `tools/contact_imports.py`: il claim dell'intento viene renderizzato e committato nella stessa richiesta, senza redirect/rilettura; la ricerca anagrafiche espone una variante `compact=1` con `load_only`, `raiseload` e limite 30, lasciando invariata l'API completa usata dalla rubrica.
- `templates/registry/contact_import_review.html`: rimuove il token dall'indirizzo con History API, usa la ricerca compatta, annulla richieste concorrenti e memorizza in pagina i risultati gia' cercati.
...
- La personalizzazione TeamSystem `500001/1` estende la vista CLIFOR con area, zona e cinque codici statistici; le descrizioni non sono ancora esposte.
- `tools/importazioni.py`: `import_anagrafiche()` esegue una sola chiamata `EVWSSYNC` a `500001/1`, poi usa la stessa risposta per i registry clienti e fornitori.
- La partizione e' rigorosa: `CF-TIPO=1` diventa `kind=customer`, `CF-TIPO=2` diventa `kind=supplier` e `CF-TIPO=0` viene ignorato. `CFCOD` e' normalizzato al formato storico a 5 cifre per entrambi; duplicati discordanti bloccano soltanto il relativo ramo prima del commit.
- La chiave univoca dei registry include `kind`, quindi lo stesso codice TeamSystem puo' identificare separatamente un cliente e un fornitore. Solo i clienti alimentano `CashCustomer` e alias.
...
- `config/celeryconfig.py`: Celery Beat accoda l'import anagrafiche ogni 30 minuti (`.00` e `.30`) con scadenza del messaggio a 25 minuti; entrambi i rami MATRIXWS vengono elaborati interamente dal worker `config.tasks.import_anagrafiche_task`.
- `tools/import_transfer_config.py`: clienti e fornitori non compaiono piu' tra i trasferimenti file configurabili, poiche' non dipendono dagli export `exp_cli.csv`/`exp_for.csv`.
...
- `CustomerRegistryMembership` e' una relazione molti-a-molti utente/cliente. Il campo `role` definisce per ogni singola associazione `administration`, `management` oppure `both`; i vecchi valori vengono normalizzati dalla migration `e1f2a3b4c5d6` senza ridurre gli accessi esistenti.
- `routes/customer_account.py` richiede la capacita' `administration`; `routes/customer_orders.py` e `tools/customer_route_reminders.py` richiedono `management`. Le viste cliente mostrano il selettore quando l'utente dispone di piu' clienti autorizzati per quella funzione.
