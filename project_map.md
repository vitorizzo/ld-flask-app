...
- `static/service-worker.js`, `templates/base.html`: il worker `v24` gestisce esplicitamente la navigazione `POST /pwa/share`, la inoltra al server con cookie e redirect abilitati e non la lascia piu' fuori dal proprio fetch handler; registrazione `mobile12` per attivazione automatica sulle installazioni esistenti.
